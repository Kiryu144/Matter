package net.andrasia.spigot.core.custom.item;
...
public class CustomItemParser
{
    ...
    public boolean parseItems(@Nonnull ConfigurationSection configurationSection)
    {
        ...
        for (String name : configurationSection.getKeys(false))
        {
            ...
            if (section == null)
            {
                Core.getInstance().getLogger().warning(String.format("Unable to parse custom item '%s' from config. Ill-formed config.", name));
                success = false;
                continue;
            }
        ...
        Core.getInstance().getLogger().info("Custom items parsed: " + Arrays.toString(itemsParsed.toArray()));
        return success;
    }
    ...
    private boolean parseItem(@Nonnull String name, @Nonnull ConfigurationSection configurationSection)
    {
        ...
        try
        {
            ...
            if (customItem == null)
            {
                Core.getInstance().getLogger().warning(String.format("Unable to instantiate custom item '%s'.", name));
                return false;
            }
